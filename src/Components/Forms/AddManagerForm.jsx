  ...
  const apiAddManager = async () => {
    try {
      const response = await axios({
        method: "POST",
        url: "http://localhost:5000/v1/api/createmanager",
        data: {
          firstName: firstName,
  ...
  const handleSubmit = async (e) => {
    ...
    toast
      .promise(
        apiAddManager(),
        {
          loading: "Creating Managers",
          success:
            "Manager Created. Please check Mailbox for Password. P.S Check Spam folder",
          error: (error) => {
            toast.error(`Error creating manager: ${error}`);
          },
        },
        {
          style: {
