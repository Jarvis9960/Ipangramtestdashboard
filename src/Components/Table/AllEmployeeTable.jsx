...
const AllEmployeeTable = () => {
  ...
  useEffect(() => {
    axios({
      method: "GET",
      url: `http://localhost:5000/v1/api/getemployees?queryField=${queryField}&sort=${sort}&page=${page}`,
      withCredentials: true,
    })
      .then((res) => {
        setEmployeeData(res.data.savedEmployee);
  ...
  const singleFetch = async (id) => {
    try {
      ...
      if (response) {
        setPopupData(response.data.Employee);
  ...
  const apiDelete = async (id) => {
    try {
      const response = await axios({
        method: "DELETE",
        url: `http://localhost:5000/v1/api/deleteemployee?employeeId=${id}`,
        withCredentials: true,
      });
                  ...
                  <tr key={curr._id}>
                    ...
                    <td className="border px-1 py-2 text-center sm:overflow-hidden xs:overflow-hidden xxs:overflow-hidden">
                      ...
                      <button
                        ...
                        onClick={() => {
                          navigate(`/udpateemployee/${curr._id}`);
                        }}
                      >
                        <FaUserEdit />
                      </button>
              ...
              <div className="backdrop-blur-md grid gap-24 grid-cols-2">
                <div className="grid gap-4 grid-cols-2">
                  ...
                  <div>
                    ...
                    <h3 className="font-bold text-2xl uppercase">
                      {popupData?.Department?.DepartmentName}
                    </h3>
                  </div>
                </div>
              </div>
